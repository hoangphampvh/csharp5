using BLL.Serviece.Implements.PaymentService.VnPay.Lib;
using System.Net;
using System.Text;
using Ultils.Helpers;

namespace BLL.Serviece.Implements.PaymentService.VnPay.Request
{
    public class VnpayPayRequest
    {
        ...
        {
            this.vnp_Locale = "vn"; //Ngôn ngữ giao diện hiển thị. Hiện tại hỗ trợ Tiếng Việt (vn), Tiếng Anh (en)
            this.vnp_IpAddr = ipAddress; //Địa chỉ IP của khách hàng thực hiện giao dịch
            this.vnp_Version = version; //Phiên bản api mà merchant kết nối. Phiên bản hiện tại là : 2.0.1 và 2.1.0
            this.vnp_CurrCode = currCode; //Đơn vị tiền tệ sử dụng thanh toán. Hiện tại chỉ hỗ trợ VND
            this.vnp_CreateDate = createDate.ToString("yyyyMMddHHmmss");
            this.vnp_TmnCode = tmnCode; //Mã website của merchant trên hệ thống của VNPAY. Ví dụ: 2QXUI4J4
            this.vnp_Amount = (int)amount * 100; // tiền
            this.vnp_Command = "pay"; //Mã API sử dụng, mã cho giao dịch thanh toán là: pay
            this.vnp_OrderType = orderType; //Mã danh mục hàng hóa
            this.vnp_OrderInfo = orderInfo; // Thông tin mô tả nội dung thanh toán
            this.vnp_ReturnUrl = returnUrl; //URL thông báo kết quả giao dịch khi Khách hàng kết thúc thanh toán.
            this.vnp_TxnRef = txnRef; //Mã tham chiếu của giao dịch tại hệ thống của merchant.
